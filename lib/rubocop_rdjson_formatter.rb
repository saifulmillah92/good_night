...
# From https://raw.githubusercontent.com/reviewdog/action-rubocop/master/rdjson_formatter/rdjson_formatter.rb
# https://docs.rubocop.org/rubocop/formatters.html
# rubocop:disable Metrics/AbcSize, Metrics/MethodLength
class RubocopRdjsonFormatter < RuboCop::Formatter::BaseFormatter
  ...
  def file_finished(file, offenses)
    offenses.each do |offense|
      ...
      next if offense.corrector # CUSTOM: skip correctable offense
  ...
  def build_diagnostic(file, offense)
...

    {
      ...
      suggestions:     build_suggestions(offense),
    }
  end
  ...
  def build_suggestions(offense)
    return [] unless offense.corrector
  ...
  def convert_path(path)
    base_path = Pathname.new(Dir.pwd.to_s)
